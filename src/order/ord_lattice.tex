...
\begin{definition}
    We give two definitions of a meet semilattice and prove that the definitions
    are equivalent.

    \begin{enumerate}
        \item A \emph{meet semilattice} is a partial order $(P, \le)$ where each
            pair of elements $x, y \in P$ have a meet (i.e. a greatest lower
            bound)~\ref{defSupremumInfimum} $x \meet y$.

        \item A \emph{meet semilattice} is an algebra $(P, \meet)$ with a binary
            meet (greatest lower bound) operation $\meet$ which satisfies
            \begin{enumerate}
                \item $x = x \meet x$ (idempotent)

                \item $x \meet y = y \meet x$ (commutative)

                \item $(x \meet y) \meet z = x \meet (y \meet z)$ (associative)
            \end{enumerate}
            for all $x, y, z \in P$.

            The relation $\le$ is defined by the algebraic operation by $x \le y
            := x = x \meet y$.
    \end{enumerate}

    \begin{proof}
        \begin{enumerate}
            \item $(1 \implies 2)$: We have to show that the meet of two
                elements in a partial order satisfies the algebraic laws of the
                second definition.

                MISSING

            \item $(2 \implies 1)$: We have to show that the algebra of the
                second definition defines a partial order where the algebraic
                operation gives the greatest lower bound consistently with the
                defined partial order.

                First we show that the operation $\le$ is a partial order i.e.
                it is reflexive, antisymmetric and transitive.

                \begin{enumerate}
                    \item reflexive: Since $\meet$ is idempotent we have $x = x
                        \meet x$. According to the definition of $\le$ in $(1)$
                        we have $x \le x$.

                    \item antisymmetric: We assume $x \le y$ and $y \le x$ and
                        have to prove $x = y$.
                        $$
                        \vertlist{
                            x &=& x \meet y &\text{definition of $x \le y$}
                            \\
                            &=& y \meet x   &\text{commutative}
                            \\
                            &=& y &\text{definition of $y \le x$}
                        }
                        $$

                    \item transitive: We assume $x \le y$ and $y \le z$ and have
                        to prove $x \le z$.
                        $$
                        \vertlist{
                            x
                            &=& x \meet y
                            &\text{definition of $x \le y$}
                            \\
                            &=& x \meet (y \meet z)
                            &\text{definition of $y \le z$}
                            \\
                            &=& (x \meet y) \meet z
                            &\text{associative}
                            \\
                        ...
                        $$
                        We have shown $x = x \meet z$ which by definition proves
                        $x \le z$.
                \end{enumerate}

                Next we show that $x \meet y$ is a lower bound of $x$. We prove
                that $x \meet y$ is a lower bound of $x$ i.e.
                $x \meet y \le x$ i.e. $x \meet y = (x \meet y) \meet x$. The
                proof that $x \meet y$ is a lower bound of $y$ is nearly the
                same.
                ...
                Finally we have to show that $x \meet y$ is the greatest lower
                bound of $x$ and $y$. We assume an element $z$ which is a lower
                bound of $x$ and $y$ (i.e. $z \le x$ and $z \le y$) and prove $z
                \le x \meet y$ i.e. $z = z \meet (x \meet y)$.
                $$
                \vertlist{
                    z
                    &=& z \meet y
                    &\text{definition of $z \le y$}
                    \\
                    &=& (z \meet x) \meet y
                    &\text{definition of $z \le x$}
                    \\
                    &=& z \meet (x \meet y)
                    &\text{associative}
                }
...
\begin{theorem}
    ...
    %--------------------------------------------------
    \emph{All chains are distributive lattices.}

    \begin{proof}
        Let $(P, \le)$ be a chain. I.e. for all $a,b \in P$ we have either $a
        \le b$ or $b \le a$. In a chain
        the join of two elements is their maximum, the meet of two elements is
        their minimum. For distributivity we have to prove
        $$
...
\paragraph{Example} Consider the following lattices.
...
%
\begin{tikzpicture}
    \graph [branch right=1cm, grow up]{
        0 [xshift=1cm]
        -- {
            a, b, c
        }
...
The for the bottom element we have $0 = \bigjoin \emptyset$. For the top element
we have $1 = a \join b$. I.e. the top and bottom elements can be generated by a
join of finitely many elements which are different from it. However none of the
elements $a$, $b$, and $c$ can be the join of other elements.
...
\begin{enumerate}
    ...
    \item In a powerset lattice $\powerset{\set{a,b,c}}$ (join is union)

        \begin{tikzpicture}
            \graph [grow up, branch right=2cm] {
                bottom/$\emptyset$[xshift=2cm]
                -- {
                    a/$\set{a}$ -- ab/$\set{a,b}$,
                    b/$\set{b}$ -- bc/$\set{b,c}$,
                    c/$\set{c}$ -- ac/$\set{a,c}$,
...
It establishes a perfect duality between distributive lattices satisfying the
descending chain condition and partial orders satisfying DCC. Note that finite
lattices or finite partial order always satisfy DCC.

Birkhoff’s Theorem tells us that a finite distributive lattice and a finite
poset are essentially the same amount of information, just "packaged"
differently:

If you give me a poset, I can build a distributive lattice (by taking its lower
sets).

If you give me a distributive lattice, I can find the poset (by looking at its
join-irreducibles).
...
$$
By definition the set $\phi(a)$ is a lower set i.e. with each join irreducible
element it contains all join irreducibles below it. Birkhoff's representation
theorem states that $\phi: L \to O(J(L))$ is a lattice isomorphism i.e. it is
bijective, order preserving and preserves all meets and joins.
...
\begin{theorem}
    ...
    \emph{
        ...
        $\phi: L \to O_f(J(L))$ be a map defined as
        %
        $\phi(a) := \set{p \in J(L): p \le a}$.
        %
        $\phi$ is a lattice isomorphism when the codmain of
    ...
    \begin{proof}
        ...
        preserves the order and preserves the joins and meets.
        ...
        \begin{enumerate}
            ...
            \item $\phi$ preserves order: We assume $a \le b$ for some $a, b \in
                L$ and have to show $\phi(a) \subseteq \phi(b)$. Assume $p \in
                \phi(a)$. By definition of $\phi$ we have $p \in J(L)$ and $p
                \le a$. Since $a \le b$ we have $p \le b$ as well by
                transitivity. Therefore $p \in \phi(b)$.

            \item $\phi$ preserves joins: Assume $c = a \join b$ for some $a, b,
                c \in L$. We have to show $\phi(c) = \phi(a) \cup \phi(b)$.

                Forward direction: Say $p \in \phi(c)$ i.e. $p \in J(L)$ and $p
                \le c$ (i.e. $p \le a \join b$). The lattice is distributive
                therefore
                by~\ref{thmDistributiveJoinIrreducibleIsJoinPrime} all join
                irreducibles are join prime. Therefore $p \le a$ or $p \le b$
                is valid. This implies $p \in \phi(a)$ or $p \in \phi(b)$.

                Backward direction: Say $p \in \phi(a)$ or $p \in \phi(b)$. We
                have to show $p \in \phi(c)$. Assume $p \in \phi(a)$ i.e. $p \in
                J(L)$ and $p \le a$. Since $c$ is an upper bound of $a$ and $b$
                we have $p \le c$, therefore $p \in \phi(c)$.

            \item $\phi$ preserves meets: We have to show $\phi(a \meet b) =
                \phi(a) \cap \phi(b)$.

                Forward direction: Assume $p \in \phi(a \meet b)$. This implies
                $p \in J(L)$, $p \le a$ and $p \le b$. Therefore $p$ is in
                $\phi(a)$ and in $\phi(b)$.

                Backward direction: Assume $p \in \phi(a) \cap \phi(b)$. This
                implies $p \in j(L)$, $p \le a$ and $p \le b$ i.e. $p$ is a
                lower bound of $a$ and $b$. Therefore $p \le a \meet b$ (which
                is the greatest lower bound) and $p \in \phi(a \meet b)$.
        \end{enumerate}
    \end{proof}
\end{theorem}


\paragraph{Why the restriction to finitely generated lower sets is important:}
For finite lattices the restriction is trivial because all lower sets are finite
aka finitely generated. The picture changes if we allow infinite sets. Consider
the set
$$
    \Nat \cup \set{\infty} = \set{0,1,2,3, \ldots, \infty}
$$
This set with the usual order is a distributive lattice satisfying the
descending chain condition. The set of join irreducibles is
$$
    P := \set{1,2,3,\ldots, \infty}
$$
The lattice of finitely generated lower sets of $P$ consists of
$$
    \emptyset, \downclosed\set{1}, \downclosed\set{2}, \downclosed\set{3},
    \ldots, \downclosed\set{\infty}
$$
which is according to Birkhoff's theorem isomorphic to the original lattice
$\set{0,1,2,3,\ldots,\infty}$. However the lattice of all possible lower sets of
$P$ includes the set
$$
    \set{1,2,3,\ldots}
$$
without the infinity element. Obviously this lower set is not finitely
generated and there is no corresponding element in the origial
lattice.

This counterexamples shows that without the restriction to finitely generated
lower sets the mapping $\phi$ in Birkhoff's representation theorem is not
surjective and therefore not an isomorphism.
...
\noindent Note that $A$, $B$ and $C$ represent the accomplishment of the tasks
and not the tasks as processes. The dependency graph says \emph{I cannot finish
$B$ before $A$ has been finished}.

We regard the dependency graph as the partial order of the join irreducibles.
The graph is finite and therefore the partial order satisfies the descending
chain condition.
...
\noindent This lattice represents all the states of the system. According to
Birkhoff's theorem this lattice is isomorphic to a lattice which contains
elements of the dependency graph as join irreducibles. In the following picture
we see the lattice with its join irreducibles highlighted in boldface.
...
\noindent $A$, $B$ and $C$ are join irreducible, the bottom element is never
join irreducible, the elements AC and ABC can be reached via joins of other
elements.

Even in this small example we see that the partial order reduced to the join
irreducibles contains all relevant information. There are only 3 join
irreducible elements. The whole state space has 6 elements. In real world
examples the partial order of join irreducibles can be handled easily, the state
space might grow exponential.
